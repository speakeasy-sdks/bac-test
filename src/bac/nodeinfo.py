...
class NodeInfo:
    ...
    def get(self) -> operations.GetNodeInfoResponse:
        ...
        hook_ctx = HookContext(operation_id='getNodeInfo', oauth2_scopes=[], security_source=None)
        ...
        res = operations.GetNodeInfoResponse(status_code=http_res.status_code, content_type=http_res.headers.get('Content-Type'), raw_response=http_res)
        
        if http_res.status_code == 200:
            if utils.match_content_type(http_res.headers.get('Content-Type'), 'application/json'):                
                out = utils.unmarshal_json(http_res.text, Optional[shared.NodeInfo])
                res.node_info = out
            else:
