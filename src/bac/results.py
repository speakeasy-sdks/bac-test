...
class Results:
    ...
    def get(self, request: shared.StateRequest) -> operations.GetResultsResponse:
        ...
        ```
        """
        hook_ctx = HookContext(operation_id='getResults', oauth2_scopes=[], security_source=None)
        ...
        res = operations.GetResultsResponse(status_code=http_res.status_code, content_type=http_res.headers.get('Content-Type'), raw_response=http_res)
        
        if http_res.status_code == 200:
            if utils.match_content_type(http_res.headers.get('Content-Type'), 'application/json'):                
                out = utils.unmarshal_json(http_res.text, Optional[shared.ResultsResponse])
                res.results_response = out
            else:
